...
#ggsave(plot = euler, here::here('figures/figure_1/fig1_venn.pdf'), height = 60, width = 60, units = "mm")



# Supplementary table of identified proteins ------------------------------

# Create a data frame of all unique proteins across groups
all_proteins <- unique(unlist(venn_list))

venn_df <- data.frame(
    protein = all_proteins,
    Male_type_I  = all_proteins %in% venn_list[["Male type I"]],
    Male_type_II = all_proteins %in% venn_list[["Male type II"]],
    Female_type_I  = all_proteins %in% venn_list[["Female type I"]],
    Female_type_II = all_proteins %in% venn_list[["Female type II"]],
    stringsAsFactors = FALSE
)


venn_df$group <- apply(venn_df[ , -1], 1, function(x) {
    groups <- names(venn_df)[-1][which(x)]   # <- notice names(venn_df)[-1]
    if (length(groups) == 0) return(NA_character_)
    paste(groups, collapse = ", ")
})

venn_table <- venn_df %>%
    select(-c(Male_type_I, Male_type_II, Female_type_I, Female_type_II))

write.csv(venn_table, here::here("supplementary_tables/supplementary_table2.csv"))
